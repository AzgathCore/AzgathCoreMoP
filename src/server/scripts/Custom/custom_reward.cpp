...
public:
    ...
    void OnUpdate(Player* player, uint32 diff)
    {
        ...
        if (timeInterval >= player->ptr_Interval)
        {
            ...
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Azgath:: Ton temps de jeu fait preuve de recompense hero !");

                PreparedStatement* stmt = AzgathEMUDatabase.GetPreparedStatement(AZGATH_UPD_BATTLEPAY_VP_COINS);
