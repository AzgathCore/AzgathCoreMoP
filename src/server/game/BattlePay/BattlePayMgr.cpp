...
void BattlePayMgr::UpdatePointsBalance(WorldSession* session, uint64 points)
{
    ...
    {
        PreparedStatement* stmt = AzgathEMUDatabase.GetPreparedStatement(AZGATH_UPD_BATTLEPAY_DECREMENT_COINS);
        stmt->setUInt32(0, points);
        stmt->setUInt32(1, session->GetAccountId());    
        AzgathEMUDatabase.Query(stmt);
    }
}
...
bool BattlePayMgr::HasPointsBalance(WorldSession* session, uint64 points)
{
    ...
    {
        PreparedStatement* stmt = AzgathEMUDatabase.GetPreparedStatement(AZGATH_SEL_BATTLEPAY_COINS);
        stmt->setUInt32(0, session->GetAccountId());
        PreparedQueryResult result_don = AzgathEMUDatabase.Query(stmt);
