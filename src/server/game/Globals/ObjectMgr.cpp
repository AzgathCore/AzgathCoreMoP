...
bool ObjectMgr::LoadTrinityStrings(const char* table, int32 min_value, int32 max_value)
{
    ...
    if (!result)
    {
        if (min_value == MIN_AZGATH_STRING_ID)              // error only in case internal strings
            TC_LOG_ERROR("server.loading", ">> Loaded 0 azgath strings. DB table `%s` is empty. Cannot continue.", table);
        else
    ...
    if (min_value == MIN_AZGATH_STRING_ID)
        TC_LOG_INFO("server.loading", ">> Loaded %u Azgath strings from table %s in %u ms", count, table, GetMSTimeDiffToNow(oldMSTime));
    else
...
const char *ObjectMgr::GetTrinityString(int32 entry, LocaleConstant locale_idx) const
{
    ...
    if (entry > 0)
        TC_LOG_ERROR("sql.sql", "Entry %i not found in `azgath_string` table.", entry);
    else
        TC_LOG_ERROR("sql.sql", "Azgath string entry %i not found in DB.", entry);
    return "<error>";
}
