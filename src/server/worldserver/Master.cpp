...
int Master::Run()
{
    ...
    TC_LOG_INFO("server.authserver", "  	  AzgathCore 5.4.8  -  Github.com/AzgathCore/AzgathCoreMoP\n");
...
bool Master::_StartDB()
{
	...
	dbString = sConfigMgr->GetStringDefault("AzgathEMUDatabaseInfo", "");
    if (dbString.empty())
    {
	...
	asyncThreads = uint8(sConfigMgr->GetIntDefault("AzgathEMUDatabase.WorkerThreads", 1));
    if (asyncThreads < 1 || asyncThreads > 32)
    {
	...
	synchThreads = uint8(sConfigMgr->GetIntDefault("AzgathEMUDatabase.SynchThreads", 1));
    ///- Initialize the FusionCMS database
    if (!AzgathEMUDatabase.Open(dbString, asyncThreads, synchThreads))
    {
